<?php
// Lire le fichier .env qui est à la racine du projet et récupérer le tableau :
...
try {
    ...
    $pdo = new PDO(
        ...
        // Data Source Name ou DSN : c'est le premier argument lors de la création de la connexion. mysql (type de base qu'on veut utiliser), host (adresse de la machine sur laquelle tourne le serveur de base de données), dbname (nom de la base à laquelle je veux me connecter), charset (encodage). Ils sont récupérés du .env grâce à $config et parse_ini_file :
        "mysql:host=" . $config['DB_HOST'] . ";dbname=" . $config['DB_NAME'] . ";charset=utf8mb4",

        // Identifiants utilisés pour se connecter à MySQL, ce sont les deux autres arguments qui doivent figurer dans cet ordre et on les récupère depuis le .env grâce à $config et parse_ini_file : 
        $config['DB_USER'],
        $config['DB_PASS']
    );
